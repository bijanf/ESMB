...
from chronos_esm.coupler.dino_coupling import LAT_LIN, co2_forcing_wm2, ocean_fluxes_jax
...
def test_co2_forcing_myhre_values():
    ...
    np.testing.assert_allclose(
        float(co2_forcing_wm2(1120.0)), 2.0 * float(co2_forcing_wm2(560.0)), rtol=1e-6
    )
...
def _synthetic():
    ...
    f = lambda s=1.0, b=0.0: jnp.asarray(
        (b + s * rng.standard_normal((nlat, nlon))).astype(np.float32)
    )
...
def test_co2_forcing_adds_heat_to_budget():
    ...
    forced = ocean_fluxes_jax(sst, u, v, ta, qa, pr, balance_heat=False, co2_ppm=560.0)[
        0
    ]
...
def test_co2_forcing_differentiable():
    ...
    np.testing.assert_allclose(
        g, 5.35 / 560.0, rtol=5e-3
    )  # d/dC [5.35 ln(C/C0)] = 5.35/C (float32)
