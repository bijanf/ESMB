...
from chronos_esm.config import OCEAN_GRID, OCEAN_DZ, EARTH_RADIUS
...
def _cell_state(scale_sign=+1.0, target_sv=15.0):
    ...
    scale = scale_sign * target_sv * 1e6 / ((np.where(up > 0, prof, 0) * dz).sum() * width)
    v = np.stack([scale * prof[k] * atl for k in range(nz)], axis=0)
    z = jnp.zeros((nz, ny, nx))
    return OceanState(u=z, v=jnp.asarray(v), w=z, temp=jnp.full((nz, ny, nx), 283.0),
                      salt=jnp.full((nz, ny, nx), 35.0), psi=jnp.zeros((ny, nx)),
                      rho=z, dic=z)
...
def test_amoc_metric_scores_clean_cell_positive():
    ...
    assert float(r["upper_cell_26N"]) == pytest.approx(15.0, abs=0.5), \
