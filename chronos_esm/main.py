...
class ModelParams(NamedTuple):
    ...
    ocean_mask_3d: Optional[jnp.ndarray] = (
        None  # 3-D wet mask (nz,ny,nx) from bathymetry
    )
...
def step_coupled(
    ...
    def atmos_land_step(carry, _):
        ...
        current_co2_ppm = params.co2_ppm * jnp.power(
            1.0 + params.co2_increase_rate, years_elapsed
        )
        co2_field = jnp.ones_like(current_state.atmos.co2) * current_co2_ppm
        current_state = current_state._replace(
            atmos=current_state.atmos._replace(co2=co2_field)
        )
        ...
        sw_toa_profile = atmos_physics.compute_solar_insolation(
            lat_rad, day_of_year, solar_constant=params.solar_constant
        )

        # Broadcast 1D profile to 2D map
        sw_down = jnp.broadcast_to(
            sw_toa_profile[:, None], current_state.atmos.temp.shape
        )
        ...
        sst_ocean_ice = (1.0 - current_state.ice.concentration) * (
            current_state.ocean.temp[0]
        ...
        next_state = current_state._replace(
            atmos=new_atmos, land=new_land, fluxes=new_fluxes, time=new_time
        )
    ...
    ocean_mask = (
        params.mask if (hasattr(params, "mask") and params.mask is not None) else None
    )
    ...
    if ocean_mask is not None:
        sst_composite = (
            ocean_mask * sst_ocean_ice + land_mask * final_atmos_state.land.temp
        )
    else:
