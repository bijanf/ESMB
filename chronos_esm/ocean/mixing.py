...
def compute_isopycnal_slopes(
    ...
    drho_dz_stable = jnp.where(
        jnp.abs(drho_dz) < SMALL_SLOPE,
        jnp.sign(drho_dz + 1e-16) * SMALL_SLOPE,
        drho_dz
    )
...
def compute_gm_bolus_velocity(
    ...
    fx_face = jnp.concatenate([jnp.zeros((1, ny, nx)), fx_face, jnp.zeros((1, ny, nx))], axis=0)
    fy_face = jnp.concatenate([jnp.zeros((1, ny, nx)), fy_face, jnp.zeros((1, ny, nx))], axis=0)
