...
from chronos_esm.atmos.dino_atmos import G, _qsat
...
def make_regridders_jax(lat_gauss, lat_lin=LAT_LIN):
    ...
    def lin_to_gauss(f_lin):  # (nlat_lin, nlon) -> (nlon, nlat_gauss)
        return jax.vmap(
            lambda col: jnp.interp(lat_gauss, lat_lin, col), in_axes=1, out_axes=0
        )(f_lin)

    def gauss_to_lin(f_g):  # (nlon, nlat_gauss) -> (nlat_lin, nlon)
        return jax.vmap(
            lambda row: jnp.interp(lat_lin, lat_gauss, row), in_axes=0, out_axes=1
        )(f_g)
...
def ocean_fluxes_jax(
    ...
    *,
    ...
    ocean_mask=None,
    sst_target=None,
    restore_tau_days=RESTORE_TAU_DAYS,
    lat_lin=LAT_LIN,
    co2_ppm=None,
    q_flux=None,
    insol_override=None,
    ...
    if insol_override is not None:
        ...
        insol = aphys.compute_solar_insolation(
            lat_lin * jnp.pi / 180.0, day_of_year=80.0
        )[:, None]
    ...
    if sst_target is not None:
        ...
        if ocean_mask is not None:
            ...
            ocean_mean = jnp.sum(jnp.where(m, net_heat * w, 0.0)) / jnp.sum(
                jnp.where(m, w, 0.0)
            )
        else:
