...
from dinosaur import time_integration as ti
from dinosaur import (
    xarray_utils,
)
...
def save_state(state, path):
    ...
    np.savez(
        ...
        sim_time=np.asarray(
            float(state.sim_time) if state.sim_time is not None else 0.0
        ),
    )
...
class DinoAtmosphere:
    ...
    def __init__(
        ...
        if orography:
            ...
            topo_lin = np.asarray(
                data.load_topography(self.nlat, self.nlon)
            )  # (nlat,nlon)
            lat_lin = np.linspace(-90, 90, self.nlat)
            topo_g = np.stack(
                [
                    np.interp(self.lat_deg, lat_lin, topo_lin[:, j])
        ...
        self.orography_modal = self.coords.horizontal.to_modal(
            aux[xarray_utils.OROGRAPHY]
        )
        ...
        self.steps_per_day = int(
            round(self.specs.nondimensionalize(1 * units.day) / self.dt)
        )
        ...
        self.v_scale_ms = float(
            self.specs.dimensionalize(1.0, units.meter / units.second).magnitude
        )
        self.t0_seconds = float(
            self.specs.dimensionalize(1.0, units.second).magnitude
        )  # s per nondim time
        ...
        self._kt = self.ka + (self.ks - self.ka) * (
            cutoff[:, None, None] * self.cos_lat**4
        )

        # initial state with a humidity tracer (init_rh * q_sat at the rest state)
        nodal_sp = jnp.exp(
            self.coords.horizontal.to_nodal(base_state.log_surface_pressure)
        )
        ...
        if seed_wind_ms > 0:
            ...
            clu = jnp.stack(
                [
                    cl * amp * jax.random.normal(k1, shp),
                    cl * amp * jax.random.normal(k2, shp),
                ]
            )
            seed_vort = self.coords.horizontal.curl_cos_lat(
                self.coords.horizontal.to_modal(clu)
            )
        ...
        self._base_state = dataclasses.replace(
            base_state,
            vorticity=base_state.vorticity + seed_vort,
        ...
        self._target_ps_mean = float(
            jnp.sum(init_ps * self._area_w) / jnp.sum(self._area_w)
        )
    ...
    def _fix_mass(self, state):
        ...
        lnps = self.coords.horizontal.to_modal(
            jnp.log(ps * (self._target_ps_mean / cur))
        )
    ...
    def _forcing_explicit(self, state, sst_nd):
        ...
        wind_ms = jnp.clip(
            jnp.sqrt(u[-1] ** 2 + v[-1] ** 2) * self.v_scale_ms, 1.0, 25.0
        )
        qsat_sst = _qsat(sst_nd, psurf_Pa)
        evap_si = (
            RHO_AIR * CE * wind_ms * jnp.maximum(qsat_sst - q[-1], 0.0)
        )  # kg/m^2/s
        dq_dt_evap = (
            evap_si * G / (psurf_Pa * self.dsigma) * self.t0_seconds
        )  # nondim 1/t
    ...
    def _run_interval(self, state, sst_nd, n_steps):
        def explicit(s):
            return _tree_add(
                self.eq.explicit_terms(s), self._forcing_explicit(s, sst_nd)
            )
    ...
    def initial_state(self, sst_nodal_K=None):
        ...
        nodal_sp = jnp.exp(
            self.coords.horizontal.to_nodal(self._base_state.log_surface_pressure)
        )
        Teq = self._equilibrium_temperature(nodal_sp, sst_nd)
    ...
    def diagnostics(self, state):
        ...
        u = self.specs.dimensionalize(
            clu[0] / self.cos_lat[None], units.meter / units.second
        ).magnitude
        v = self.specs.dimensionalize(
            clu[1] / self.cos_lat[None], units.meter / units.second
        ).magnitude
        ...
        nodal_sp = np.exp(
            np.asarray(self.coords.horizontal.to_nodal(state.log_surface_pressure))
        )
        ps_Pa = nodal_sp[0] * self.p_scale_Pa  # (nlon, nlat)
        q = np.asarray(
            self.coords.horizontal.to_nodal(state.tracers["specific_humidity"])
        )
        q = np.maximum(q, 0.0)
        ...
        precip = np.sum(
            cond_si * (ps_Pa * self.dsigma) / G, axis=0
        )  # kg/m^2/s (nlon,nlat)
