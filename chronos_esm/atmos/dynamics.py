...
def step_atmos(
    ...
    psi, _ = solve_poisson_2d(
        state.vorticity, dx, dy, max_iter=50, tol=1e-3, x0=state.psi
    )
    chi, _ = solve_poisson_2d(
        state.divergence, dx, dy, max_iter=50, tol=1e-3, x0=state.chi
    )
    ...
    heating_rad = physics.compute_radiative_forcing(
        state.temp, co2_mean, lat_rad=lat_rad
    )
    ...
    forcing_u = drag_u
    forcing_v = drag_v
    forcing_t = (heating_precip + heating_rad) / CP + flux_sensible / (
        1000.0 * 1000.0
    )  # Approx density/height
