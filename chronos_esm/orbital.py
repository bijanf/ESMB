...
ORBIT_PI = OrbitalParams(obliquity_deg=23.459, eccentricity=0.016764,
                         long_perihelion_deg=280.33)   # 1850 CE (perihelion-180 = 100.33)
ORBIT_6KA = OrbitalParams(obliquity_deg=24.105, eccentricity=0.018682,
                          long_perihelion_deg=180.87)  # mid-Holocene (perihelion-180 = 0.87)
...
def daily_insolation(lat_rad, solar_longitude_deg, orbit=ORBIT_PRESENT,
    ...
    s = (solar_constant / jnp.pi) * dist * (
        h0 * jnp.sin(lat_rad) * jnp.sin(decl)
        + jnp.cos(lat_rad) * jnp.cos(decl) * jnp.sin(h0))
    return jnp.maximum(s, 0.0)


def solar_longitude_from_day(day_of_year, orbit=ORBIT_PRESENT, vernal_equinox_day=80.0,
                             n_iter=8):
    ...
    def true_to_mean(nu):
        E = 2.0 * jnp.arctan(jnp.tan(nu / 2.0) * jnp.sqrt((1 - e) / (1 + e)))  # eccentric anom
...
def annual_mean_insolation(lat_rad, orbit=ORBIT_PRESENT, solar_constant=SOLAR_CONSTANT,
                           n=360):
    ...
    S = jnp.stack([daily_insolation(lat, float(L), orbit, solar_constant) for L in lam])  # (n,nlat)
    return jnp.sum(S * w[:, None], axis=0) / jnp.sum(w)


__all__ = ["OrbitalParams", "ORBIT_PI", "ORBIT_6KA", "ORBIT_PRESENT", "SOLAR_CONSTANT",
           "daily_insolation", "solar_longitude_from_day", "annual_mean_insolation"]
