...
def run_carbon_test():
    ...
    print(f"Atmos: {c_atmos_0:.2f} -> {c_atmos_1:.2f} (Delta: {c_atmos_1 - c_atmos_0:.2f})")
    print(f"Ocean: {c_ocean_0:.2f} -> {c_ocean_1:.2f} (Delta: {c_ocean_1 - c_ocean_0:.2f})")
    print(f"Land:  {c_land_0:.2f} -> {c_land_1:.2f} (Delta: {c_land_1 - c_land_0:.2f})")
    
    total_0 = c_atmos_0 * 1.0 # Need proper conversion factors for strict conservation check
