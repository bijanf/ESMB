...
def run_control(years: float = 1.0):
    ...
    def run_one_month(start_state):
        ...
        # Scan
        (final_s, final_acc), _ = jax.lax.scan(month_step_fn, (start_state, init_acc), jnp.arange(steps_per_month))
    ...
    for month in range(1, n_months + 1):
        ...
        logger.info(f"Month {month} complete in {t_end - t_start:.2f}s. T_mean={t_mean:.2f}K.")
        ...
        mean_ocean = current_state.ocean._replace(
            temp=means.ocean_temp, salt=means.ocean_salt,
            u=means.ocean_u, v=means.ocean_v, w=means.ocean_w, psi=means.ocean_psi,
            dic=means.ocean_dic
        )
        mean_atmos = current_state.atmos._replace(
            temp=means.atmos_temp, u=means.atmos_u, v=means.atmos_v, q=means.atmos_q,
            co2=means.atmos_co2, vorticity=means.atmos_vorticity,
            divergence=means.atmos_divergence, ln_ps=means.atmos_ln_ps
        )
