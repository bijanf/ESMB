...
def load_latest_state(output_dir: Path):
    ...
    ocean = veros_driver.OceanState(
        ...
        w=jnp.zeros_like(jnp.array(ds.ocean_u.values)), # w usually diagnostic or not saved?
    ...
    ice = ice_driver.IceState(
        ...
        surface_temp=jnp.ones_like(jnp.array(ds.ice_thickness.values)) * -1.8 # Approx or save?
    )
    ...
    fluxes = coupled_state.FluxState(
        ...
        sst=jnp.array(ds.atmos_temp.values) # Approx SST from Atmos T? Or use Ocean Surface T
    )
...
def resume_run(target_years: float = 100.0):
    ...
    def run_one_month(start_state):
        final_s, _ = jax.lax.scan(month_step_fn, start_state, jnp.arange(steps_per_month))
        return final_s
    ...
    for month in range(start_month + 1, n_months + 1):
        ...
        logger.info(f"Month {month} complete in {t_end - t_start:.2f}s. T_mean={t_mean:.2f}K. Saving output...")
        model_io.save_state_to_netcdf(current_state, output_dir / f"state_{month:04d}.nc")
