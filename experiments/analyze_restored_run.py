...
def analyze_restored_run():
    # 1. Setup paths and output files
    input_pattern = "outputs/control_run_restored/mean_*.nc"
    ...
    plt.xlabel('Months')
    plt.ylabel('AMOC (Sv)')
    plt.title(f'Restored Run AMOC Strength (Months 1-{len(files)})')
    plt.legend()
    plt.grid(True)
    plt.savefig('restored_amoc_timeseries.png')
    print("Saved restored_amoc_timeseries.png")
    ...
    plt.title(f'Restored Run Global Temperature (Months 1-{len(files)})')
    plt.legend()
    plt.grid(True)
    plt.savefig('restored_t_mean_timeseries.png')
    print("Saved restored_t_mean_timeseries.png")
    ...
    try:
        ...
        for f in last_files:
            ...
            if ds_clim is None:
                ...
                ds_clim = ds_clim + ds # adds variables
            count += 1
            # Don't close immediately if using lazy loading, but netcdf4 engine usually loads eager or is file handle safe
            # With netcdf4 engine and open_dataset, likely eager-ish or okay.
        ...
        if 'sst' in ds_clim or 'ocean_temp' in ds_clim:
            ...
            plt.savefig('restored_sst_map.png')
            print("Saved restored_sst_map.png")
        ...
        if 'ocean_salt' in ds_clim:
            ...
            plt.savefig('restored_sss_map.png')
            print("Saved restored_sss_map.png")
